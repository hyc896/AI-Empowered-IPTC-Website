...
"""
中国社会科学网-哲学频道采集器
CSSN Philosophy Channel Collector

数据来源：http://phil.cssn.cn/
特点：专门的哲学理论文章，经常用马克思主义原理分析问题
"""
...
from backend.database.entities import CssnPhilMessage
...
class CssnPhilCollector(PlaywrightCollectorBase):
    """中国社会科学网-哲学频道采集器（马克思主义哲学理论）"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.url = config['config'].get('url', 'http://phil.cssn.cn/')
        ...
        if _chroma_available:
            ...
            logger.warning("【中国社科网-哲学】ChromaDB不可用")

        if _llm_available:
            self.embedding_client = get_embedding_client()
        else:
            self.embedding_client = None
            logger.warning("【中国社科网-哲学】Embedding服务不可用")

        if _field_enricher_available:
            self.field_enricher = get_field_enricher()
        else:
            self.field_enricher = None
            logger.warning("【中国社科网-哲学】FieldEnricher不可用")
    ...
    async def _on_initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【中国社科网-哲学】采集器初始化成功")
            return True
        except Exception as e:
            logger.error(f"【中国社科网-哲学】初始化失败: {e}")
            return False
    ...
    async def _collect_once(self) -> None:
        ...
        try:
            ...
            logger.info(f"【中国社科网-哲学】已存在 {len(existing_urls)} 条URL")

            articles = await self._scrape_articles(existing_urls)
            if not articles:
                logger.debug("【中国社科网-哲学】无新文章")
                return

            logger.info(f"【中国社科网-哲学】抓取到 {len(articles)} 篇新文章")
            await self._preprocess_items(articles)
            await self._store_items(articles)
            logger.info(f"【中国社科网-哲学】采集完成")

        except Exception as e:
            logger.error(f"【中国社科网-哲学】采集错误: {e}", exc_info=True)

    async def _get_existing_urls(self, limit: int = 1000) -> set:
        """获取已存在URL集合"""
        try:
            with create_session() as db:
                results = db.query(CssnPhilMessage.url).filter(
                    CssnPhilMessage.source_id == self.source_id,
                    CssnPhilMessage.url.isnot(None)
                ).order_by(
                    CssnPhilMessage.crawled_at.desc()
                ).limit(limit).all()
                return {str(row[0]) for row in results if row[0]}
        except Exception as e:
            logger.error(f"【中国社科网-哲学】获取existing_urls失败: {e}")
            return set()
    ...
    async def _scrape_articles(self, existing_urls: set) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for selector in selectors_to_try:
                try:
                    ...
                    if test_elements and len(test_elements) > 0:
                        ...
                        logger.debug(f"【中国社科网-哲学】使用选择器: {selector}")
                        break
                except Exception:
                    continue

            if not selected_selector:
                logger.warning("【中国社科网-哲学】未找到文章元素")
                return []
                ...
                for idx in range(processed_count, current_count):
                    ...
                    try:
                        ...
                        if article_url.startswith('/'):
                            article_url = f'http://phil.cssn.cn{article_url}'
                        elif not article_url.startswith('http'):
                            continue

                        # 去重检查
                        if article_url in existing_urls:
                            logger.debug(f"【中国社科网-哲学】遇到已存在URL，停止")
                            return articles
                        ...
                        logger.error(f"【中国社科网-哲学】提取失败: {e}")
                        continue
            ...
            logger.error(f"【中国社科网-哲学】抓取失败: {e}", exc_info=True)
        finally:
            if page:
                await page.close()
    ...
    async def _fetch_article_content(self, page: Page, url: str) -> str:
        ...
        try:
            ...
            logger.error(f"【中国社科网-哲学】获取正文失败: {e}")
            return ""
    ...
    async def _preprocess_items(self, items: List[Dict[str, Any]]) -> None:
        ...
        logger.info(f"【中国社科网-哲学】开始预处理 {len(items)} 条")
        ...
        for idx, item in enumerate(items, 1):
            ...
            try:
                ...
                logger.error(f"【中国社科网-哲学】预处理失败: {e}")
                item['region'] = self.region
                item['industry_tags'] = None
                item['ai_tag'] = None
            ...
            with create_session() as db:
                for item in items:
                    ...
                    try:
                        db.add(message)
                        db.commit()
                        logger.debug(f"【中国社科网-哲学】MySQL存储成功")
                    except IntegrityError as e:
                        db.rollback()
                        if "Duplicate entry" in str(e):
                            logger.debug(f"【中国社科网-哲学】重复URL")

        except Exception as e:
            logger.error(f"【中国社科网-哲学】MySQL存储失败: {e}")
    ...
    async def _store_to_chroma(self, items: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.error(f"【中国社科网-哲学】ChromaDB存储失败: {e}")
