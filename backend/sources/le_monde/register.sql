-- Le Monde Pixels采集器注册脚本
-- 执行命令（Linux）：
-- mysql -h localhost -P 3306 -u root -p123456 --default-character-set=utf8mb4 message_platform < backend/sources/le_monde/register.sql

USE message_platform;

-- 1. 创建消息表
CREATE TABLE IF NOT EXISTS mp_le_monde_messages (
    ...
    language VARCHAR(10) DEFAULT 'fr' COMMENT '语言',
...
INSERT INTO mp_message_sources (
    ...
    'le_monde',
    'news',
    'Le Monde Pixels（法国世界报）',
    JSON_OBJECT(
        'mysql_table', 'mp_le_monde_messages',
        'chroma_collection', 'le_monde_tech',
        'region', '法国',
        'language', 'fr',
        'rss_url', 'https://www.lemonde.fr/pixels/rss_full.xml',
        'collector_module', 'backend.sources.le_monde.collector',
...
WHERE name = 'Le Monde Pixels';

-- 4. 检查表是否创建成功
SHOW CREATE TABLE mp_le_monde_messages\G
