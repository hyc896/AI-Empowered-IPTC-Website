...
"""
Le Monde Pixels Technology News Collector
世界报科技新闻采集器（基于RSS Feed）

数据来源：
- RSS Feed: https://www.lemonde.fr/pixels/rss_full.xml
- 格式：标准RSS 2.0
- 频率：每次约20条新闻
- 地区：法国
- 语言：法语（需要翻译成中文）
...
from backend.database.entities import LeMondeMessage
...
class LeMondeCollector:
    """Le Monde Pixels科技新闻采集器（RSS Feed模式）"""

    RSS_FEED_URL = "https://www.lemonde.fr/pixels/rss_full.xml"
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: 配置字典，包含：
                ...
                - config.region: 默认地区
                - config.language: 语言（fr=法语）
        """
        self.source_id = config.get('id', 'auto')
        self.interval = config.get('interval', 3600)

        config_detail = config.get('config', {})
        self.mysql_table = config_detail.get('mysql_table', 'mp_le_monde_messages')
        self.chroma_collection = config_detail.get('chroma_collection', 'le_monde_tech')
        self.region = config_detail.get('region', '法国')
        self.language = config_detail.get('language', 'fr')

        if _chroma_available:
            self.chroma_storage = get_chromadb_storage()
        else:
            self.chroma_storage = None
            logger.warning("【LeMonde】ChromaDB不可用，将只存储到MySQL")
        ...
        if _llm_available:
            ...
            logger.warning("【LeMonde】LLM服务不可用，将跳过向量化、翻译和字段增强")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【LeMonde】采集器初始化成功")
            return True
        except Exception as e:
            logger.error(f"【LeMonde】采集器初始化失败: {e}")
            return False
    ...
    async def run(self) -> None:
        ...
        if not await self.initialize():
            logger.error("【LeMonde】采集器初始化失败，退出")
            return

        self._running = True
        logger.info(f"【LeMonde】采集器已启动 (采集间隔={self.interval}秒)")

        while self._running:
            try:
                await self._collect_once()
            except Exception as e:
                logger.error(f"【LeMonde】采集失败: {e}", exc_info=True)
    ...
    async def _collect_once(self) -> None:
        ...
        """
        logger.info(f"【LeMonde】开始采集RSS feed")
        ...
        try:
            ...
            if not feed.entries:
                logger.warning(f"【LeMonde】RSS feed为空 (Bozo: {feed.bozo}, Status: {feed.get('status', 'N/A')})")
                if feed.bozo and hasattr(feed, 'bozo_exception'):
                    logger.error(f"【LeMonde】RSS解析错误: {feed.bozo_exception}")
                return

            logger.info(f"【LeMonde】RSS feed解析成功，共 {len(feed.entries)} 条记录")

            latest_url = await self._get_latest_stored_url()
            logger.debug(f"【LeMonde】最新存储URL: {latest_url}")
            ...
            for entry in feed.entries:
                ...
                if latest_url and url == latest_url:
                    logger.info(f"【LeMonde】遇到已存在URL，停止采集: {url}")
                    break
                ...
                if published_parsed:
                    try:
                        ...
                        logger.warning(f"【LeMonde】解析发布时间失败: {e}")
            ...
            if not articles:
                logger.info("【LeMonde】没有新文章需要采集")
                return

            logger.info(f"【LeMonde】发现 {len(articles)} 篇新文章，开始预处理...")

            await self._preprocess_and_store(articles)

            logger.info(f"【LeMonde】本次采集完成，共处理 {len(articles)} 篇文章")

        except Exception as e:
            logger.error(f"【LeMonde】采集过程出错: {e}", exc_info=True)
    ...
    async def _get_latest_stored_url(self) -> Optional[str]:
        ...
        try:
            with create_session() as db:
                latest = db.query(LeMondeMessage).filter(
                    LeMondeMessage.source_id == self.source_id,
                    LeMondeMessage.url.isnot(None)
                ).order_by(
                    LeMondeMessage.published_at.desc()
                ).first()
            ...
            logger.error(f"【LeMonde】获取最新URL失败: {e}")
            return None
    ...
    async def _preprocess_and_store(self, items: List[Dict[str, Any]]) -> None:
            ...
            try:
                ...
                if self.translator and summary_en:
                    try:
                        summary_zh = await self.translator.translate(
                            summary_en,
                            context="Le Monde Pixels法国科技新闻摘要（从法语翻译成中文）"
                        )
                    except Exception as e:
                        logger.warning(f"【LeMonde】翻译失败，使用原文: {e}")
                        summary_zh = summary_en
                ...
                if self.field_enricher:
                    try:
                        ...
                        logger.debug(f"【LeMonde】字段增强成功: region={region_str}, industry_tags={industry_tags_str}, ai_tag={ai_tag_str}")
                    except Exception as e:
                        logger.warning(f"【LeMonde】字段增强失败，使用默认值: {e}")
                ...
                logger.error(f"【LeMonde】预处理失败: {e}", exc_info=True)
                continue

        if not processed_items:
            logger.warning("【LeMonde】预处理后无有效数据")
            return
        ...
        if self.chroma_storage and self.chroma_storage.is_initialized():
            try:
                ...
                logger.error(f"【LeMonde】向量化失败: {e}", exc_info=True)
        ...
        with create_session() as db:
            for item in items:
                try:
                    ...
                    db.rollback()
                    duplicate_count += 1
                    logger.debug(f"【LeMonde】重复记录（URL已存在）: {item['url']}")
                except Exception as e:
                    db.rollback()
                    logger.error(f"【LeMonde】存储失败: {e}", exc_info=True)

        logger.info(f"【LeMonde】MySQL存储完成: 成功={stored_count}, 重复={duplicate_count}")
    ...
    async def _sync_to_chromadb(self, items: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"【LeMonde】ChromaDB同步完成: {len(ids)} 条记录")

        except Exception as e:
            logger.error(f"【LeMonde】ChromaDB同步失败: {e}", exc_info=True)
