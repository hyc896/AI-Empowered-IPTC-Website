...
from backend.database.entities import CSISMessage
...
class CSISCollector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.url = config['config'].get('url', 'https://www.csis.org/topics/artificial-intelligence')
        ...
        if _chroma_available:
            ...
            logger.warning("【CSIS】ChromaDB不可用，将只存储到MySQL")
        ...
        if _llm_available:
            ...
            logger.warning("【CSIS】LLM服务不可用，将跳过向量化")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【CSIS】采集器初始化成功: {self.url}")
            return True
        except Exception as e:
            logger.error(f"【CSIS】采集器初始化失败: {e}")
            return False
    ...
    async def run(self) -> None:
        ...
        if not await self.initialize():
            logger.error("【CSIS】采集器初始化失败，退出")
            return

        self._running = True
        logger.info(f"【CSIS】采集器已启动 (采集间隔={self.interval}秒)")

        while self._running:
            try:
                await self._collect_once()
            except Exception as e:
                logger.error(f"【CSIS】采集失败: {e}")
    ...
    async def _collect_once(self) -> None:
        ...
        try:
            ...
            if new_items:
                # 访问详情页获取完整content
                logger.info(f"【CSIS】开始访问 {len(new_items)} 篇文章的详情页...")
                ...
                logger.info(f"【CSIS】采集到 {len(new_items)} 篇新文章")
            else:
                logger.debug("【CSIS】所有文章已存在，无新数据")

        except Exception as e:
            logger.error(f"【CSIS】采集错误: {e}", exc_info=True)
    ...
    async def _get_latest_stored_url(self) -> Optional[str]:
        ...
        try:
            with create_session() as db:
                latest = db.query(CSISMessage).filter(
                    CSISMessage.source_id == self.source_id,
                    CSISMessage.url.isnot(None)
                ).order_by(
    ...
    async def _scrape_articles_list(self, latest_url: Optional[str]) -> List[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"【CSIS】第 {attempt + 1} 次尝试失败: {e}，{retry_delay}秒后重试...")
                    if page:
                        await page.close()
                        page = None
                    await asyncio.sleep(retry_delay)
                else:
                    logger.error(f"【CSIS】所有重试均失败: {e}", exc_info=True)
                    if page:
                        await page.close()
                    return []
        ...
        try:
            ...
            logger.info(f"【CSIS】全量加载完成，共 {final_count} 篇文章")
    ...
    async def _fetch_article_content(self, article_url: str) -> Optional[str]:
        ...
        try:
            ...
            # 提取文章正文内容
            paragraphs = await detail_page.query_selector_all('article p, .post-body p, .field-name-body p, .article-body p')
            content_parts = []
            ...
            for para in paragraphs:
                ...
                if para_text and len(para_text) > 20:
                    # 排除作者信息、分享按钮等
                    if not any(keyword in para_text.lower() for keyword in ['share', 'follow us', 'subscribe', 'newsletter']):
    ...
    async def _extract_article_from_element(self, article_elem) -> Optional[Dict[str, Any]]:
        """
        从.views-row元素中提取文章数据

        Args:
            article_elem: .views-row DOM元素
        ...
        try:
            # 提取标题和链接
            title_elem = await article_elem.query_selector('h2 a, h3 a, .views-field-title a')
            if not title_elem:
                return None
            ...
            # 补全相对URL
            if url.startswith('/'):
                url = f"https://www.csis.org{url}"

            # 提取内容类型
            category_elem = await article_elem.query_selector('.field-name-field-content-type, .content-type')
            category = ""
            if category_elem:
                category = (await category_elem.inner_text()).strip()

            # 提取作者
            author_elem = await article_elem.query_selector('.field-name-field-authors, .author')
            author = ""
            if author_elem:
                author = (await author_elem.inner_text()).strip()

            # 提取日期
            date_elem = await article_elem.query_selector('.field-name-post-date, time, .date')
            published_at = None
            if date_elem:
            ...
            # 提取摘要
            excerpt_elem = await article_elem.query_selector('.views-field-body, .field-name-body, .summary')
            excerpt = ""
            if excerpt_elem:
            ...
            return {
                ...
                "url": url,
                "region": self.region,
                "category": category if category else "AI Policy",
