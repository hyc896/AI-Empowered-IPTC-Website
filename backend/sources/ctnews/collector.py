...
"""
中国旅游新闻网采集器
China Tourism News Collector

数据来源：https://www.ctnews.com.cn/
特点：官方军事新闻，权威军事资讯
"""
...
from backend.database.entities import CtnewsMessage
...
class CtnewsCollector(PlaywrightCollectorBase):
    """中国旅游新闻网采集器"""

    def __init__(self, config: Dict[str, Any]):
        """初始化采集器"""
        super().__init__(config)
        self.url = config.get('url', 'https://www.ctnews.com.cn/')
        self.max_articles = config.get('max_articles', 20)
        self.source_id = config['id']

        logger.info(f"【中国旅游新闻网】采集器初始化成功")
    ...
    async def _collect_once(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not articles:
                logger.info(f"【中国旅游新闻网】没有新文章需要采集")
            ...
            logger.info(f"【中国旅游新闻网】采集完成，成功保存 {saved_count} 篇文章")
            return {'success': True, 'collected_count': saved_count}

        except Exception as e:
            logger.error(f"【中国旅游新闻网】采集失败: {e}", exc_info=True)
    ...
    async def _get_existing_urls(self) -> set:
        ...
        try:
            with create_session() as db:
                results = db.query(CtnewsMessage.url).all()
                return {row[0] for row in results}
        except Exception as e:
            logger.error(f"【中国旅游新闻网】获取已存在URL失败: {e}")
            return set()
    ...
    async def _scrape_articles(self, existing_urls: set) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"【中国旅游新闻网】访问: {self.url}")
            ...
            logger.info(f"【中国旅游新闻网】找到 {len(news_links)} 个链接")
            ...
            for link in news_links:
                ...
                try:
                    ...
                    # 处理相对链接
                    if href.startswith('/'):
                        href = f"https://www.ctnews.com.cn{href}"
                    ...
                    logger.warning(f"【中国旅游新闻网】提取链接失败: {e}")
                    continue

            logger.info(f"【中国旅游新闻网】采集到 {len(articles)} 篇新文章")

        except Exception as e:
            logger.error(f"【中国旅游新闻网】抓取失败: {e}", exc_info=True)
        finally:
            if page:
                await page.close()
    ...
    async def _enrich_fields(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(articles), CONCURRENT_LIMIT):
            ...
            logger.info(f"【中国旅游新闻网】处理批次 {batch_num}/{total_batches}，本批 {len(batch)} 篇")
            ...
            for result in results:
                if isinstance(result, dict):
                    ...
                    logger.error(f"【中国旅游新闻网】字段补全异常: {result}")

        logger.info(f"【中国旅游新闻网】字段补全完成，成功 {len(enriched)}/{len(articles)} 篇")
        return enriched
    ...
    async def _enrich_single_article(self, article: Dict[str, Any], index: int, total: int) -> Dict[str, Any]:
        ...
        page = None
        try:
            logger.debug(f"【中国旅游新闻网】补全字段 ({index}/{total}): {article['title'][:30]}")
            ...
            logger.error(f"【中国旅游新闻网】字段补全失败 ({index}/{total}): {e}")
            return None
        finally:
            if page:
                await page.close()
    ...
    async def _extract_content(self, page: Page) -> str:
        ...
        try:
            ...
            logger.error(f"【中国旅游新闻网】提取正文失败: {e}")
            return ""
    ...
    async def _extract_publish_time(self, page: Page) -> datetime:
        ...
        try:
            ...
            logger.error(f"【中国旅游新闻网】提取发布时间失败: {e}")
            return None
    ...
    def _parse_time(self, time_text: str) -> datetime:
        ...
        try:
            ...
            logger.error(f"【中国旅游新闻网】解析时间失败: {e}")
            return None
                ...
                for article in articles:
                    try:
                        message = CtnewsMessage(
                            ...
                            provider=article.get('provider', '中国旅游新闻网'),
                        ...
                        db.rollback()
                        logger.debug(f"【中国旅游新闻网】URL已存在，跳过: {article['url']}")
                    except Exception as e:
                        db.rollback()
                        logger.error(f"【中国旅游新闻网】保存失败: {e}")

        except Exception as e:
            logger.error(f"【中国旅游新闻网】数据库存储失败: {e}")
