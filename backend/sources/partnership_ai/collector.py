...
class PartnershipAICollector:
    ...
    async def run(self) -> None:
        ...
        if not await self.initialize():
            logger.error("【Partnership AI】采集器初始化失败，退出")
            return

        self._running = True
        logger.info(f"【Partnership AI】采集器已启动 (采集间隔={self.interval}秒)")

        while self._running:
            try:
                await self._collect_once()
            except Exception as e:
                logger.error(f"【Partnership AI】采集失败: {e}")
    ...
    async def stop(self) -> None:
        ...
        logger.info("【Partnership AI】采集器已停止")
