-- Times of India Tech采集器注册脚本
-- 执行命令（Linux）：
-- mysql -h localhost -P 3306 -u root -p123456 --default-character-set=utf8mb4 message_platform < backend/sources/times_of_india/register.sql

USE message_platform;

-- 1. 创建消息表
CREATE TABLE IF NOT EXISTS mp_times_of_india_messages (
...
INSERT INTO mp_message_sources (
    ...
    'times_of_india',
    'news',
    'Times of India Tech（印度时报）',
    JSON_OBJECT(
        'mysql_table', 'mp_times_of_india_messages',
        'chroma_collection', 'times_of_india_tech',
        'region', '印度',
        'language', 'en',
        'rss_url', 'https://timesofindia.indiatimes.com/rssfeeds/66949542.cms',
        'collector_module', 'backend.sources.times_of_india.collector',
...
WHERE name = 'Times of India Tech';

-- 4. 检查表是否创建成功
SHOW CREATE TABLE mp_times_of_india_messages\G
