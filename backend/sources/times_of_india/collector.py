...
"""
TimesOfIndia Technology News Collector
TimesOfIndia科技新闻采集器（基于RSS Feed）

数据来源：
- RSS Feed: https://timesofindia.indiatimes.com/rssfeeds/66949542.cms
- 格式：标准RSS 2.0
- 频率：每次约20条新闻
- 地区：印度（G20国家）
- 语言：英语
...
class TimesOfIndiaCollector:
    """TimesOfIndia Technology新闻采集器（RSS Feed模式）"""

    RSS_FEED_URL = "https://timesofindia.indiatimes.com/rssfeeds/66949542.cms"
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.mysql_table = config_detail.get('mysql_table', 'mp_times_of_india_messages')
        self.chroma_collection = config_detail.get('chroma_collection', 'times_of_india_tech')
        self.region = config_detail.get('region', '印度')
        ...
        if _chroma_available:
            ...
            logger.warning("【TimesOfIndia】ChromaDB不可用，将只存储到MySQL")
        ...
        if _llm_available:
            ...
            logger.warning("【TimesOfIndia】LLM服务不可用，将跳过向量化、翻译和字段增强")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【TimesOfIndia】采集器初始化成功")
            return True
        except Exception as e:
            logger.error(f"【TimesOfIndia】采集器初始化失败: {e}")
            return False
    ...
    async def run(self) -> None:
        ...
        if not await self.initialize():
            logger.error("【TimesOfIndia】采集器初始化失败，退出")
            return

        self._running = True
        logger.info(f"【TimesOfIndia】采集器已启动 (采集间隔={self.interval}秒)")

        while self._running:
            try:
                await self._collect_once()
            except Exception as e:
                logger.error(f"【TimesOfIndia】采集失败: {e}", exc_info=True)
    ...
    async def stop(self) -> None:
        """停止采集器"""
        self._running = False
        logger.info("【TimesOfIndia】采集器已停止")
    ...
    async def _collect_once(self) -> None:
        ...
        """
        logger.info(f"【TimesOfIndia】开始采集RSS feed")
        ...
        try:
            ...
            if not feed.entries:
                logger.warning(f"【TimesOfIndia】RSS feed为空 (Bozo: {feed.bozo}, Status: {feed.get('status', 'N/A')})")
                if feed.bozo and hasattr(feed, 'bozo_exception'):
                    logger.error(f"【TimesOfIndia】RSS解析错误: {feed.bozo_exception}")
                return

            logger.info(f"【TimesOfIndia】RSS feed解析成功，共 {len(feed.entries)} 条记录")

            latest_url = await self._get_latest_stored_url()
            logger.debug(f"【TimesOfIndia】最新存储URL: {latest_url}")
            ...
            for entry in feed.entries:
                ...
                if latest_url and url == latest_url:
                    logger.info(f"【TimesOfIndia】遇到已存在URL，停止采集: {url}")
                    break
                ...
                if published_parsed:
                    try:
                        ...
                        logger.warning(f"【TimesOfIndia】解析发布时间失败: {e}")
            ...
            if not articles:
                logger.info("【TimesOfIndia】没有新文章需要采集")
                return

            logger.info(f"【TimesOfIndia】发现 {len(articles)} 篇新文章，开始预处理...")

            await self._preprocess_and_store(articles)

            logger.info(f"【TimesOfIndia】本次采集完成，共处理 {len(articles)} 篇文章")

        except Exception as e:
            logger.error(f"【TimesOfIndia】采集过程出错: {e}", exc_info=True)
        ...
        try:
            with create_session() as db:
                latest = db.query(TimesOfIndiaMessage).filter(
                    TimesOfIndiaMessage.source_id == self.source_id,
                    TimesOfIndiaMessage.url.isnot(None)
                ).order_by(
            ...
            logger.error(f"【TimesOfIndia】获取最新URL失败: {e}")
            return None
    ...
    async def _fetch_article_content(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"【TimesOfIndia】详情页访问失败 (status={response.status_code}): {url}")
                return None

            soup = BeautifulSoup(response.content, 'html.parser')

            # Times of India使用div._s30J作为文章主体容器
            article_body = soup.find('div', class_='_s30J')

            if not article_body:
                logger.warning(f"【TimesOfIndia】未找到文章内容容器: {url}")
                return None

            # 移除不需要的元素
            for tag in article_body.find_all(['script', 'style', 'iframe', 'img', 'figure']):
                tag.decompose()
            ...
            logger.debug(f"【TimesOfIndia】成功提取文章内容: {len(text)} 字符")
            return text.strip()

        except Exception as e:
            logger.warning(f"【TimesOfIndia】获取详情页失败: {e}")
            return None
    ...
    async def _preprocess_and_store(self, items: List[Dict[str, Any]]) -> None:
            ...
            try:
                ...
                if not full_content:
                    ...
                    if not full_content or len(full_content) < 50:
                        logger.warning(f"【TimesOfIndia】跳过无内容文章: {item['title']}")
                        continue

                # summary使用RSS描述（如有），否则用content前1000字
                summary_en = item.get('content', '') or full_content[:1000]
                ...
                if self.translator and summary_en:
                    try:
                        summary_zh = await self.translator.translate(
                            summary_en,
                            context="TimesOfIndia科技新闻摘要"
                        )
                    except Exception as e:
                        logger.warning(f"【TimesOfIndia】翻译失败，使用原文: {e}")
                        summary_zh = summary_en

                # 更新item的content为完整文章内容
                ...
                if self.field_enricher:
                    try:
                        ...
                        logger.debug(f"【TimesOfIndia】字段增强成功: region={region_str}, industry_tags={industry_tags_str}, ai_tag={ai_tag_str}")
                    except Exception as e:
                        logger.warning(f"【TimesOfIndia】字段增强失败，使用默认值: {e}")
                ...
                logger.error(f"【TimesOfIndia】预处理失败: {e}", exc_info=True)
                continue

        if not processed_items:
            logger.warning("【TimesOfIndia】预处理后无有效数据")
            return
        ...
        if self.chroma_storage and self.chroma_storage.is_initialized():
            try:
                ...
                logger.error(f"【TimesOfIndia】向量化失败: {e}", exc_info=True)
        ...
        with create_session() as db:
            for item in items:
                try:
                    ...
                    db.rollback()
                    duplicate_count += 1
                    logger.debug(f"【TimesOfIndia】重复记录（URL已存在）: {item['url']}")
                except Exception as e:
                    db.rollback()
                    logger.error(f"【TimesOfIndia】存储失败: {e}", exc_info=True)

        logger.info(f"【TimesOfIndia】MySQL存储完成: 成功={stored_count}, 重复={duplicate_count}")
    ...
    async def _sync_to_chromadb(self, items: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"【TimesOfIndia】ChromaDB同步完成: {len(ids)} 条记录")

        except Exception as e:
            logger.error(f"【TimesOfIndia】ChromaDB同步失败: {e}", exc_info=True)
