-- CNBC Technology采集器注册脚本
-- 执行命令（Linux）：
-- mysql -h localhost -P 3306 -u root -p123456 --default-character-set=utf8mb4 message_platform < backend/sources/cnbc/register.sql

USE message_platform;

-- 1. 创建消息表
CREATE TABLE IF NOT EXISTS mp_cnbc_messages (
...
INSERT INTO mp_message_sources (
    ...
    JSON_OBJECT(
        'mysql_table', 'mp_cnbc_messages',
        'chroma_collection', 'cnbc_tech',
        'region', '美国',
        'language', 'en',
        'rss_url', 'https://www.cnbc.com/id/19854910/device/rss/rss.html',
        'collector_module', 'backend.sources.cnbc.collector',
...
WHERE name = 'CNBC Technology';

-- 4. 检查表是否创建成功
SHOW CREATE TABLE mp_cnbc_messages\G
