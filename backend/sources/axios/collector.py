...
"""
Axios News Collector
Axios新闻采集器（基于RSS Feed）
...
class AxiosCollector:
    """Axios新闻采集器（RSS Feed模式）"""

    RSS_FEED_URL = "https://api.axios.com/feed/"
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.mysql_table = config_detail.get('mysql_table', 'mp_axios_messages')
        self.chroma_collection = config_detail.get('chroma_collection', 'axios_news')
        ...
        if _chroma_available:
            ...
            logger.warning("【Axios】ChromaDB不可用，将只存储到MySQL")
        ...
        if _llm_available:
            ...
            logger.warning("【Axios】LLM服务不可用，将跳过向量化、翻译和字段增强")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【Axios】采集器初始化成功")
            return True
        except Exception as e:
            logger.error(f"【Axios】采集器初始化失败: {e}")
            return False
    ...
    async def run(self) -> None:
        ...
        if not await self.initialize():
            logger.error("【Axios】采集器初始化失败，退出")
            return

        self._running = True
        logger.info(f"【Axios】采集器已启动 (采集间隔={self.interval}秒)")

        while self._running:
            try:
                await self._collect_once()
            except Exception as e:
                logger.error(f"【Axios】采集失败: {e}", exc_info=True)
    ...
    async def _collect_once(self) -> None:
        ...
        """
        logger.info(f"【Axios】开始采集RSS feed")
        ...
        try:
            ...
            if not feed.entries:
                logger.warning(f"【Axios】RSS feed为空 (Bozo: {feed.bozo}, Status: {feed.get('status', 'N/A')})")
                if feed.bozo and hasattr(feed, 'bozo_exception'):
                    logger.error(f"【Axios】RSS解析错误: {feed.bozo_exception}")
                return

            logger.info(f"【Axios】RSS feed解析成功，共 {len(feed.entries)} 条记录")

            latest_url = await self._get_latest_stored_url()
            logger.debug(f"【Axios】最新存储URL: {latest_url}")
            ...
            for entry in feed.entries:
                ...
                if latest_url and url == latest_url:
                    logger.info(f"【Axios】遇到已存在URL，停止采集: {url}")
                    break
                ...
                if published_parsed:
                    try:
                        ...
                        logger.warning(f"【Axios】解析发布时间失败: {e}")
            ...
            if not articles:
                logger.info("【Axios】没有新文章需要采集")
                return

            logger.info(f"【Axios】发现 {len(articles)} 篇新文章，开始预处理...")

            await self._preprocess_and_store(articles)

            logger.info(f"【Axios】本次采集完成，共处理 {len(articles)} 篇文章")

        except Exception as e:
            logger.error(f"【Axios】采集过程出错: {e}", exc_info=True)
        ...
        try:
            with create_session() as db:
                latest = db.query(AxiosMessage).filter(
                    AxiosMessage.source_id == self.source_id,
                    AxiosMessage.url.isnot(None)
                ).order_by(
            ...
            logger.error(f"【Axios】获取最新URL失败: {e}")
            return None
    ...
    async def _preprocess_and_store(self, items: List[Dict[str, Any]]) -> None:
            ...
            try:
                ...
                if self.translator and summary_en:
                    try:
                        ...
                        logger.warning(f"【Axios】翻译失败，使用原文: {e}")
                        summary_zh = summary_en
                ...
                if self.field_enricher:
                    try:
                        ...
                        logger.debug(f"【Axios】字段增强成功: region={region_str}, industry_tags={industry_tags_str}, ai_tag={ai_tag_str}")
                    except Exception as e:
                        logger.warning(f"【Axios】字段增强失败，使用默认值: {e}")
                ...
                logger.error(f"【Axios】预处理失败: {e}", exc_info=True)
                continue

        if not processed_items:
            logger.warning("【Axios】预处理后无有效数据")
            return
        ...
        if self.chroma_storage and self.chroma_storage.is_initialized():
            try:
                ...
                logger.error(f"【Axios】向量化失败: {e}", exc_info=True)
        ...
        with create_session() as db:
            for item in items:
                try:
                    ...
                    db.rollback()
                    duplicate_count += 1
                    logger.debug(f"【Axios】重复记录（URL已存在）: {item['url']}")
                except Exception as e:
                    db.rollback()
                    logger.error(f"【Axios】存储失败: {e}", exc_info=True)

        logger.info(f"【Axios】MySQL存储完成: 成功={stored_count}, 重复={duplicate_count}")
    ...
    async def _sync_to_chromadb(self, items: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"【Axios】ChromaDB同步完成: {len(ids)} 条记录")

        except Exception as e:
            logger.error(f"【Axios】ChromaDB同步失败: {e}", exc_info=True)
