...
"""
Wired Technology News Collector
Wired科技新闻采集器（基于RSS Feed）
...
class WiredCollector:
    """Wired Technology新闻采集器（RSS Feed模式）"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if _chroma_available:
            ...
            logger.warning("【Wired】ChromaDB不可用，将只存储到MySQL")
        ...
        if _llm_available:
            ...
            logger.warning("【Wired】LLM服务不可用，将跳过向量化、翻译和字段增强")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【Wired】采集器初始化成功")
            return True
        except Exception as e:
            logger.error(f"【Wired】采集器初始化失败: {e}")
            return False
    ...
    async def run(self) -> None:
        ...
        if not await self.initialize():
            logger.error("【Wired】采集器初始化失败，退出")
            return

        self._running = True
        logger.info(f"【Wired】采集器已启动 (采集间隔={self.interval}秒)")

        while self._running:
            try:
                await self._collect_once()
            except Exception as e:
                logger.error(f"【Wired】采集失败: {e}", exc_info=True)
    ...
    async def _collect_once(self) -> None:
        ...
        """
        logger.info(f"【Wired】开始采集RSS feed")
        ...
        try:
            ...
            if not feed.entries:
                logger.warning(f"【Wired】RSS feed为空 (Bozo: {feed.bozo}, Status: {feed.get('status', 'N/A')})")
                if feed.bozo and hasattr(feed, 'bozo_exception'):
                    logger.error(f"【Wired】RSS解析错误: {feed.bozo_exception}")
                return

            logger.info(f"【Wired】RSS feed解析成功，共 {len(feed.entries)} 条记录")

            latest_url = await self._get_latest_stored_url()
            logger.debug(f"【Wired】最新存储URL: {latest_url}")
            ...
            for entry in feed.entries:
                ...
                if latest_url and url == latest_url:
                    logger.info(f"【Wired】遇到已存在URL，停止采集: {url}")
                    break
                ...
                if published_parsed:
                    try:
                        ...
                        logger.warning(f"【Wired】解析发布时间失败: {e}")
            ...
            if not articles:
                logger.info("【Wired】没有新文章需要采集")
                return

            logger.info(f"【Wired】发现 {len(articles)} 篇新文章，开始预处理...")

            await self._preprocess_and_store(articles)

            logger.info(f"【Wired】本次采集完成，共处理 {len(articles)} 篇文章")

        except Exception as e:
            logger.error(f"【Wired】采集过程出错: {e}", exc_info=True)
    ...
    async def _get_latest_stored_url(self) -> Optional[str]:
        ...
        try:
            with create_session() as db:
                latest = db.query(WiredMessage).filter(
                    WiredMessage.source_id == self.source_id,
                    WiredMessage.url.isnot(None)
                ).order_by(
            ...
            logger.error(f"【Wired】获取最新URL失败: {e}")
            return None
    ...
    async def _preprocess_and_store(self, items: List[Dict[str, Any]]) -> None:
            ...
            try:
                ...
                if self.translator and summary_en:
                    try:
                        ...
                        logger.warning(f"【Wired】翻译失败，使用原文: {e}")
                        summary_zh = summary_en
                ...
                if self.field_enricher:
                    try:
                        ...
                        logger.debug(f"【Wired】字段增强成功: region={region_str}, industry_tags={industry_tags_str}, ai_tag={ai_tag_str}")
                    except Exception as e:
                        logger.warning(f"【Wired】字段增强失败，使用默认值: {e}")
                ...
                logger.error(f"【Wired】预处理失败: {e}", exc_info=True)
                continue

        if not processed_items:
            logger.warning("【Wired】预处理后无有效数据")
            return
        ...
        if self.chroma_storage and self.chroma_storage.is_initialized():
            try:
                ...
                logger.error(f"【Wired】向量化失败: {e}", exc_info=True)
        ...
        with create_session() as db:
            for item in items:
                try:
                    ...
                    db.rollback()
                    duplicate_count += 1
                    logger.debug(f"【Wired】重复记录（URL已存在）: {item['url']}")
                except Exception as e:
                    db.rollback()
                    logger.error(f"【Wired】存储失败: {e}", exc_info=True)

        logger.info(f"【Wired】MySQL存储完成: 成功={stored_count}, 重复={duplicate_count}")
    ...
    async def _sync_to_chromadb(self, items: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"【Wired】ChromaDB同步完成: {len(ids)} 条记录")

        except Exception as e:
            logger.error(f"【Wired】ChromaDB同步失败: {e}", exc_info=True)
