-- Wired Technology新闻采集器注册脚本
-- 执行命令（PowerShell）：
-- Get-Content backend/sources/wired/register.sql -Encoding UTF8 | mysql -h localhost -P 3306 -u root -p123456 --default-character-set=utf8mb4 message_platform

USE message_platform;

-- 1. 创建消息表
CREATE TABLE IF NOT EXISTS mp_wired_messages (
    ...
    region VARCHAR(200) COMMENT '地区（中文格式，从文章内容提取，默认"美国"）',
...
INSERT INTO mp_message_sources (
    ...
    'Wired科技新闻（美国权威科技媒体）',
    JSON_OBJECT(
        'mysql_table', 'mp_wired_messages',
        'chroma_collection', 'wired_tech',
        'region', '美国',
        'language', 'en',
        'rss_url', 'https://www.wired.com/feed/rss',
        'collector_module', 'backend.sources.wired.collector',
        'description', 'Wired是美国权威科技媒体，专注科技、文化和商业交叉领域深度报道'
    ),
    '0 */1 * * *',
    1,
    NOW(),
...
-- 4. 检查表是否创建成功
SHOW CREATE TABLE mp_wired_messages\G
