...
"""
求是网采集器
Qiushi Net Collector

数据来源：http://www.qstheory.cn/
特点：中共中央机关刊《求是》杂志主办的理论宣传平台
"""
...
from backend.database.entities import QstheoryMessage
...
class QstheoryCollector(PlaywrightCollectorBase):
    """求是网采集器（国内思政理论新闻）"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.url = config['config'].get('url', 'http://www.qstheory.cn/')
    ...
    async def _on_initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"【求是网】采集器初始化成功")
            return True
        except Exception as e:
            logger.error(f"【求是网】初始化失败: {e}")
            return False
    ...
    async def _collect_once(self) -> None:
        ...
        try:
            ...
            logger.info(f"【求是网】已存在 {len(existing_urls)} 条URL")

            articles = await self._scrape_articles(existing_urls)
            if not articles:
                logger.debug("【求是网】无新文章")
                return

            logger.info(f"【求是网】抓取到 {len(articles)} 篇新文章")
            await self._preprocess_items(articles)
            await self._store_items(articles)
            logger.info(f"【求是网】采集完成")

        except Exception as e:
            logger.error(f"【求是网】采集错误: {e}", exc_info=True)

    async def _get_existing_urls(self, limit: int = 1000) -> set:
        """获取已存在URL集合"""
        try:
            with create_session() as db:
                results = db.query(QstheoryMessage.url).filter(
                    QstheoryMessage.source_id == self.source_id,
                    QstheoryMessage.url.isnot(None)
                ).order_by(
                    QstheoryMessage.crawled_at.desc()
                ).limit(limit).all()
                return {str(row[0]) for row in results if row[0]}
        except Exception as e:
            logger.error(f"【求是网】获取existing_urls失败: {e}")
            return set()
    ...
    async def _scrape_articles(self, existing_urls: set) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for selector in selectors_to_try:
                try:
                    ...
                    if test_elements and len(test_elements) > 0:
                        ...
                        logger.debug(f"【求是网】使用选择器: {selector}")
                        break
                except Exception:
                    continue

            if not selected_selector:
                logger.warning("【求是网】未找到文章元素")
                return []
                ...
                for idx in range(processed_count, current_count):
                    ...
                    try:
                        ...
                        if article_url.startswith('/'):
                            article_url = f'http://www.qstheory.cn{article_url}'
                        elif not article_url.startswith('http'):
                            continue

                        # 去重检查
                        if article_url in existing_urls:
                            logger.debug(f"【求是网】遇到已存在URL，停止")
                            return articles
                        ...
                        # 发布时间
                        time_element = await element.query_selector('.time, .date')
            ...
            logger.error(f"【求是网】抓取失败: {e}", exc_info=True)
        finally:
            if page:
                await page.close()
    ...
    async def _preprocess_items(self, items: List[Dict[str, Any]]) -> None:
        ...
        logger.info(f"【求是网】开始预处理 {len(items)} 条")
            ...
            with create_session() as db:
                for item in items:
                    ...
                    try:
                        db.add(message)
                        db.commit()
                        logger.debug(f"【求是网】MySQL存储成功")
                    except IntegrityError as e:
                        db.rollback()
                        if "Duplicate entry" in str(e):
                            logger.debug(f"【求是网】重复URL")

        except Exception as e:
            logger.error(f"【求是网】MySQL存储失败: {e}")
    ...
    async def _store_to_chroma(self, items: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.error(f"【求是网】ChromaDB存储失败: {e}")
