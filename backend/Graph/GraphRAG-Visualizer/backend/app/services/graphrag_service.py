...
graphrag_path = Path(__file__).parent.parent.parent.parent.parent / "GraphRAG"
...
class GraphRAGService:
    ...
    async def build_graph(
        ...
        # 创建或更新 Book 节点
        await self._create_book_node(file_id, filename, len(entities), len(relations))

        return stats

    async def _create_book_node(self, file_id: str, filename: str, entity_count: int, relation_count: int):
        """创建或更新书籍节点"""
        if not self.storage:
            return

        query = """
        MERGE (b:Book {id: $file_id})
        SET b.name = $filename,
        ...
        await self.storage.execute_write_async(
            query,
            file_id=file_id,
            filename=filename,
    ...
    async def get_all_books(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            RETURN b.id as book_id,
                   ...
                   toString(b.updated_at) as upload_time
            ORDER BY b.updated_at DESC
            """

            results = await self.storage.execute_read_async(query, {})
            ...
            print(f"获取书籍列表失败: {str(e)}")
            return []

    async def get_graph_by_book(self, book_id: str) -> Dict[str, Any]:
        """
        获取指定书籍的知识图谱
        ...
        query = """
        MATCH (m:Message)
        WHERE m.id STARTS WITH $book_id
        MATCH (m)-[:MENTIONS]->(e:Entity)
        OPTIONAL MATCH (e)-[r]->(e2:Entity)
        RETURN e, r, e2
        """

        results = await self.storage.execute_read_async(query, {"book_id": book_id})
    ...
    async def get_node_subgraph(self, node_id: str, book_id: Optional[str] = None, depth: int = 1) -> Dict[str, Any]:
        """
        获取以指定节点为中心的子图
        ...
        if book_id:
            ...
            MATCH (m:Message)
            WHERE m.id STARTS WITH $book_id
            MATCH (m)-[:MENTIONS]->(center:Entity {{name: $node_id}})
            MATCH path = (center)-[*1..{depth}]-(e:Entity)
            WHERE (m)-[:MENTIONS]->(e)
            WITH center, e, relationships(path) as rels
            UNWIND rels as r
            RETURN DISTINCT center, e, r, startNode(r) as source, endNode(r) as target
            """
        else:
            query = f"""
        ...
        results = await self.storage.execute_read_async(query, params)
        ...
        for record in results:
            ...
            if center:
                center_dict = dict(center) if hasattr(center, '__iter__') and not isinstance(center, str) else center
                center_name = center_dict.get('name') if isinstance(center_dict, dict) else str(center)
                center_type = center_dict.get('type', 'unknown') if isinstance(center_dict, dict) else 'unknown'

                if center_name not in node_ids:
                    nodes.append({
                        "id": center_name,
                        "name": center_name,
                        "category": center_type,
                        "value": center_dict.get('mention_count', 1) if isinstance(center_dict, dict) else 1,
                        "symbolSize": min(40 + (center_dict.get('mention_count', 1) if isinstance(center_dict, dict) else 1) * 5, 100),
                        "isCenter": True
                    })
            ...
            if source and target and relation:
                source_dict = dict(source) if hasattr(source, '__iter__') and not isinstance(source, str) else source
                target_dict = dict(target) if hasattr(target, '__iter__') and not isinstance(target, str) else target
                source_name = source_dict.get('name') if isinstance(source_dict, dict) else str(source)
                target_name = target_dict.get('name') if isinstance(target_dict, dict) else str(target)
                relation_type = relation.type if hasattr(relation, 'type') else 'RELATED_TO'
