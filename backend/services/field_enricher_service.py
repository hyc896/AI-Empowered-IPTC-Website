...
class FieldEnricherService:
    ...
    def _build_region_prompt(self, title: str, content: str) -> str:
        ...
        prompt = f"""请分析以下新闻内容，判断其主要涉及的地区。

判断规则：
1. 如果是全球性事件（国际油价、黄金价格、全球气候、国际组织等），返回"全球"
2. 如果明确涉及某个国家，返回国家名（中文）
3. 如果涉及某个国家的特定省份/州，返回"国家/省份"格式（用斜杠分隔）
4. 如果涉及某个城市，返回"国家/省份/城市"格式（用斜杠分隔）
5. 如果涉及某个跨国组织或区域，返回最相关的实体（如"欧盟"、"东盟"）
...
仅返回地区信息（如"中国"、"美国/加利福尼亚州"、"全球"），不要任何解释。"""
